...
@TeleOp(name = "5741 TeleOp Willem", group = "Competition")
public class LifterBotWillem extends OpMode {
    ...
    public void loop() {                                        // goes into loop after the setup is done  in above void
        ...
        double strafe = -gamepad1.right_stick_x;                 // input joystick values into variables that we can use to control the motors
        double drive = gamepad1.right_stick_y;
        double liftSpeed = gamepad1.left_stick_y;
        ...
        if (gamepad1.right_trigger > 0.1 && grip1.getPosition() < 0.9) {
            grip1.setPosition(grip1.getPosition() + 0.005);
            grip2.setPosition(grip2.getPosition() - 0.005);
        } else if (gamepad1.left_trigger > 0.1 && grip1.getPosition() > 0) {
            grip1.setPosition(grip1.getPosition() - 0.005);
            grip2.setPosition(grip2.getPosition() + 0.005);
        }
        ...
        telemetry.addData("Grip1", "Position:  " + String.format("%.2s", grip1.getPosition())); // feedback given to the driver phone from the robot phone
        telemetry.addData("Grip2", "Position:  " + String.format("%.2s", grip2.getPosition()));
    }
}